...
export default async function SuperAdminFormTemplatesPage() {
  const session = await auth();
  const orgId = session!.user.organizationId ?? "";

  // Fetch all templates with their branch info
  const [templates, branches] = await Promise.all([
    db.formTemplate.findMany({
      where: { organizationId: orgId },
      orderBy: [{ branchId: "asc" }, { isDefault: "desc" }, { updatedAt: "desc" }],
    }),
    db.branch.findMany({
      where: { organizationId: orgId },
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Manage admission forms for all branches"
        breadcrumbs={[{ label: "Super Admin", href: "/super-admin" }, { label: "Form Templates" }]}
        actions={
          <Button asChild>
      ...
      {templates.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {templates.map((tpl) => {
            ...
            const branchName = tpl.branchId
              ? (branchMap.get(tpl.branchId) ?? "Unknown Branch")
              : "All Branches";
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <span className="text-[10px] font-medium text-[#1B4332]">
                          {tpl.isDefault ? "Default · " : ""}{branchName}
                        </span>
                      </div>
                    </div>
