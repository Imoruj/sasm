  ...
  try {
    ...
    for (const payment of pendingPayments) {
      try {
        ...
        // "pending" state from Paystack — leave for next cron run
      } catch (err) {
    ...
    console.log(
      `[CRON_VERIFY] Processed ${pendingPayments.length} payments: ${resolved} resolved, ${failed} failed`
    );
    return NextResponse.json({ checked: pendingPayments.length, resolved, failed });
  } catch (err) {
    console.error("[CRON_VERIFY] Fatal error:", err);
